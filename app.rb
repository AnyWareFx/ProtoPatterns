...
%w(customer offer product product_detail product_offer shopping_cart shopping_cart_item).each do |model|
  require "#{Dir.pwd}/models/#{model}"
end

Dir["#{Dir.pwd}/routes/*.rb"].each do |route|
  require route
end
...
DataMapper.setup(:default, ENV['DATABASE_URL'] || "sqlite3://#{Dir.pwd}/data/development.db")
...
[
  ...
  product = Product.create :name => product_spec[:name],
...
[
  ...
  offer = Offer.create :name => offer_spec[:name],
                       :unit_price => offer_spec[:unit_price]

  offer_spec[:products].each do |product_spec|
    product = Product.first :name => product_spec[:name]
...
[
  ...
  customer = Customer.create :first_name => customer_spec[:first_name],
